...
### npm
* v2 严重的问题：1、依赖包重复安装；2、嵌套层级过深

  ![包管理工具-11](./images/%E5%8C%85%E7%AE%A1%E7%90%86%E5%B7%A5%E5%85%B7-11.jpg)

* v3 为解决v2版本的问题，在此版本做出了**扁平化**处理。

  ![包管理工具-12](./images/%E5%8C%85%E7%AE%A1%E7%90%86%E5%B7%A5%E5%85%B7-12.jpg)
  
  * 新问题：
  
  1、无法彻底解决依赖包重复安装
  
  2、有另依赖：被提升上来的依赖为幽灵依赖。被提升至顶层导致项目可以直接使用，但事实上项目未对此依赖进行管理，当此依赖发生变化了却无感知。
  
  3、依赖不等幂：安装顺序不同会造成包的结构不同，会造成不同使用者不同时间，不同环境的项目有所差异。
